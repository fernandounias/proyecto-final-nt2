...
const Info = ({game}) => {
  ...
  const priceBuyStyle = {...divStyle, color: 'white', padding: '10px 15px', display: 'flex', alignItems: 'center', justifyContent: 'space-evenly', background:'#1976d2'}
  ...
  return (
  <div style={{width:'36%'}}>
    <div style={{margin: '20px', width:'90%', background: 'grey', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '10px'}}>
      <img src={game.image} alt="image" style={{maxWidth: '100%', borderRadius: '10px'}}/>
    </div>
    ...
    <div style={divStyle}>
      ...
      </div>
      {
        ...
        {game.genres.map((genre) => (
          <Chip id={genre.id} label={genre.description} style={tagStyle} size='small' variant='outlined' onClick={handleLink}></Chip>
        ))}
        </div>
      }
    </div>
